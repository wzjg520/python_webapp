...
class VersionField(Field):
    ...
    def __init__(self, name=None):
        super(VersionField, self).__init__(name=name, defalut=0, ddl='bigint')

_triggers = frozenset(['pre_insert', 'pre_update', 'pre_delete'])


def _gen_sql(table_name, mapppings):
    pk = None
    sql = ['-- generating SQL for %s' %
           table_name, 'create table `%s` (' % table_name]
    for f in sorted(mapppings.values(), lambda x, y: cmp(x._order, y._order)):
        ...
        for k, v in attrs.iteritems():
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    if primary_key:
                        raise TypeError(
                            'Cannt defined more than 1 primary key in class: %s' % name)
                    if v.updateable:
                        logging.warning(
                            'NOTE: change primary key to non-updateable.')
                        v.updateable = False
                    if v.nullable:
                        logging.warning(
...
class Model(dict):
    ...
    def __getattr__(self, key):
        try:
            ...
            raise AttributeError(r"'Dict' object has noattribute '%s'." % key)
